...
class 'Katarina'
	...
	function Katarina:GetDrawText(unit)
		...
		elseif self.spells.R:Ready() and DmgTable.Q + DmgTable.W + DmgTable.E + (DmgTable.R *10) > unit.health then
...
function GetWardSlot()
	wardItems = {3340,3350,3361,3362,3154,3160,2045,2049,2044,2043}	
	for i, item in ipairs(wardItems) do
		slot=GetSlotItem(item)
		if(slot ~= nil) and myHero:CanUseSpell(slot) ~= 5 then
			return slot
		end
	end	
end
