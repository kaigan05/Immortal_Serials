...
namespace ImmortalSerials.Evade
{
    internal static class SkillshotDetector
    {
        ...
        {
            if (!sender.IsValid || !Debug && sender.Team == ObjectManager.Player.Team)
            {
                return;
            }
            ...
            {
                Skillshot skillshot = DetectedSkillShots[i];
        ...
        private static void ObjSpellMissileOnOnCreate(GameObject sender, EventArgs args)
        {
            ...
            Obj_AI_Base unit = missile.SpellCaster;
            if (!unit.IsValid || (unit.Team == ObjectManager.Player.Team) && !Debug)
            {
                return;
            }

            SpellData spellData = SpellDatabase.GetByMissileName(missile.SData.Name);
            if (spellData == null)
            {
                return;
            }
            Vector2 missilePosition = missile.Position.To2D();
            Vector2 unitPosition = missile.StartPosition.To2D();
            Vector2 endPos = missile.EndPosition.To2D();

            //Calculate the real end Point:
            Vector2 direction = (endPos - unitPosition).Normalized();
            ...
            int castTime = Environment.TickCount - Game.Ping / 2 - (spellData.MissileDelayed ? 0 : spellData.Delay) -
        ...
        private static void ObjSpellMissileOnOnDelete(GameObject sender, EventArgs args)
        {
            ...
            string spellName = missile.SData.Name;
        ...
        private static void ObjAiHeroOnOnProcessSpellCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            ...
            if (!sender.IsValid || sender.Team == ObjectManager.Player.Team&&!Debug)
            {
                return;
            }
            //Get the skillshot data.
            SpellData spellData = SpellDatabase.GetByName(args.SData.Name);
            ...
            {
                foreach (GameObject o in ObjectManager.Get<GameObject>())
                {
            ...
            if (spellData.FromObjects != null && spellData.FromObjects.Length > 0)
            {
                foreach (GameObject obj in ObjectManager.Get<GameObject>())
                {
                    ...
                    {
                        Vector2 start = obj.Position.To2D();
                        Vector2 end = start + spellData.Range * (args.End.To2D() - obj.Position.To2D()).Normalized();
                        TriggerOnDetectSkillshot(
        ...
        public static void GameOnOnGameProcessPacket(GamePacketEventArgs args)
        {
            ...
            if (args.PacketData[0] == 0x3B)
            {
                ...
                float missileSpeed = packet.ReadFloat();
                ...
                SpellData spellData = SpellDatabase.GetBySpeed(unit.ChampionName, (int) missileSpeed, id);
                ...
                int castTime = Environment.TickCount - Game.Ping / 2 - spellData.Delay -
                               (int)
