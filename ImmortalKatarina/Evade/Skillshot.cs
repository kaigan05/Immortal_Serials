...
namespace ImmortalSerials.Evade
{
    ...
    public class Skillshot
    {
        ...
        {
            int t = Math.Max(0, Environment.TickCount + time - StartTick - SpellData.Delay);
        ...
        public Vector2 GetMissilePosition(int time)
        {
            ...
            {
                float t1 = (SpellData.MissileAccel > 0
        ...
        public bool IsSafeToBlink(Vector2 point, int timeOffset, int delay = 0)
        {
            ...
            if (SpellData.Type == SkillShotType.SkillshotMissileLine)
            {
                Vector2 missilePositionAfterBlink = GetMissilePosition(delay + timeOffset);
                LeagueSharp.Common.Geometry.ProjectionInfo myPositionProjection =
            ...
            int timeToExplode = SpellData.ExtraDuration + SpellData.Delay +
            ...
            Obj_AI_Base unit = null)
        {
            ...
            for (int i = 0; i <= path.Count - 2; i++)
            {
                ...
                {
                    Vector2 sideStart = Polygon.Points[j];
                    Vector2 sideEnd = Polygon.Points[j == (Polygon.Points.Count - 1) ? 0 : j + 1];

                    LeagueSharp.Common.Geometry.IntersectionResult intersection = from.Intersection(
                        to, sideStart, sideEnd);
                ...
                List<FoundIntersection> sortedList = segmentIntersections.OrderBy(o => o.Distance).ToList();
                ...
                SpellData.Type == SkillShotType.SkillshotMissileCone)
            {
                ...
                if (IsSafe(ObjectManager.Player.ServerPosition.To2D()))
                {
                    ...
                    for (int i = 0; i <= allIntersections.Count - 1; i = i + 2)
                    {
                        FoundIntersection enterIntersection = allIntersections[i];
                        Vector2 enterIntersectionProjection = enterIntersection.Point.ProjectOn(Start, End).SegmentPoint;

                        //Intersection with no exit point.
                        if (i == allIntersections.Count - 1)
                        {
                            Vector2 missilePositionOnIntersection =
                        ...
                        FoundIntersection exitIntersection = allIntersections[i + 1];
                        Vector2 exitIntersectionProjection = exitIntersection.Point.ProjectOn(Start, End).SegmentPoint;

                        Vector2 missilePosOnEnter = GetMissilePosition(enterIntersection.Time - timeOffset);
            ...
            int timeToExplode = (SpellData.DontAddExtraDuration ? 0 : SpellData.ExtraDuration) + SpellData.Delay +
                                (int) (1000 * Start.Distance(End) / SpellData.MissileSpeed) -
                                (Environment.TickCount - StartTick);
            ...
            Vector2 myPositionWhenExplodesWithOffset = Geometry.PositionAfter(path, timeToExplode, speed, timeOffset);
        ...
        public bool IsAboutToHit(int time, Obj_AI_Base unit)
        {
            if (SpellData.Type == SkillShotType.SkillshotMissileLine)
            {
                ...
                LeagueSharp.Common.Geometry.ProjectionInfo projection = unit.ServerPosition.To2D()
        ...
        public void Draw(System.Drawing.Color color, System.Drawing.Color missileColor, int width = 1)
        {
