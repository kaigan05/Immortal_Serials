...
namespace ImmortalSerials.Evade
{
    ...
    internal static class Collision
    {
        ...
        {
            float tDelay = delay / 1000f + (from.Distance(unit) / speed);
            float d = tDelay * unit.MoveSpeed;
            List<Vector2> path = unit.GetWaypoints();
        ...
        public static Vector2 GetCollisionPoint(Skillshot skillshot)
        {
            ...
            foreach (CollisionObjectTypes cObject in skillshot.SpellData.CollisionObjects)
            {
                switch (cObject)
                {
                    ...
                    case CollisionObjectTypes.YasuoWall:
                        ...
                        string level = wall.Name.Substring(wall.Name.Length - 6, 1);
                        int wallWidth = (300 + 50 * Convert.ToInt32(level));


                        Vector2 wallDirection = (wall.Position.To2D() - YasuoWallCastedPos).Normalized().Perpendicular();
                        Vector2 wallStart = wall.Position.To2D() + wallWidth / 2 * wallDirection;
                        Vector2 wallEnd = wallStart - wallWidth * wallDirection;
                        Geometry.Polygon wallPolygon = new Geometry.Rectangle(wallStart, wallEnd, 75).ToPolygon();
                        var intersections = new List<Vector2>();

                        for (int i = 0; i < wallPolygon.Points.Count; i++)
                        {
                            LeagueSharp.Common.Geometry.IntersectionResult inter =
                        ...
                        {
                            Vector2 intersection = intersections.OrderBy(item => item.Distance(@from)).ToList()[0];
                            float collisionT = Environment.TickCount +
                                               Math.Max(
                                                   0,
            ...
            Vector2 result = collisions.Count > 0 ? collisions.OrderBy(c => c.Distance).ToList()[0].Position : new Vector2();
