...
namespace ImmortalSerials.Evade
{
    public static class Utils
    {
        ...
        public static Obj_AI_Base Closest(List<Obj_AI_Base> targetList, Vector2 from)
        {
            ...
            foreach (Obj_AI_Base target in targetList)
            {
                float distance = Vector2.DistanceSquared(from, target.ServerPosition.To2D());
                if (distance < dist)
                {
                    dist = distance;
                    result = target;
        ...
        {
            Vector2 from = Drawing.WorldToScreen(start);
            Vector2 to = Drawing.WorldToScreen(end);
