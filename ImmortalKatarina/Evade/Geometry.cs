...
namespace ImmortalSerials.Evade
{
    ...
    public static class Geometry
    {
        ...
        {
            int distance = Math.Max(0, t - delay) * speed / 1000;
        ...
        public class Circle
        {
            ...
            public Polygon ToPolygon(int offset = 0, float overrideWidth = -1)
            {
                ...
                {
                    double angle = i * 2 * Math.PI / CircleLineSegmentN;
        ...
        public class Polygon
        {
            ...
            public void Draw(Color color, int width = 1)
            {
                ...
                {
                    int nextIndex = (Points.Count - 1 == i) ? 0 : (i + 1);
        ...
        public class Ring
        {
            ...
            public Polygon ToPolygon(int offset = 0)
            {
                ...
                float outRadius = (offset + Radius + RingRadius) / (float) Math.Cos(2 * Math.PI / CircleLineSegmentN);
                float innerRadius = Radius - RingRadius - offset;
        ...
        public class Sector
        {
            ...
            {
                var result = new Polygon();
                float outRadius = (Radius + offset) / (float) Math.Cos(2 * Math.PI / CircleLineSegmentN);

                result.Add(Center);
                Vector2 side1 = Direction.Rotated(-Angle * 0.5f);

                for (int i = 0; i <= CircleLineSegmentN; i++)
                {
                    Vector2 cDirection = side1.Rotated(i * Angle / CircleLineSegmentN).Normalized();
